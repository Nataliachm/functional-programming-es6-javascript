...
const ferraris = (cars) => {
    const arrFerrari = cars.filter(cars => cars.brand == "Ferrari")
    return arrFerrari
}


const nineties = (cars) => {
    const arrayNineties = cars.filter(cars => 1980 < cars.model && 1990 > cars.model)
    return arrayNineties
}


const list = (cars) => {
    const listTotal = cars.map(cars => `This ${cars.brand} from ${cars.model} costs ${cars.price}`)
    return listTotal
}
...
const bmwSum = (cars) => {
    const carsBmw = cars.filter(cars => cars.brand == "BMW")
    const sumaPrices = carsBmw.reduce(
        (ac, valor) => ac + valor.price, 0)
    return sumaPrices
}
